...
import {
  ...
  finishReedsModelRun,
  failReedsModelRun,
  type ReedsModelGovernanceContext,
} from "./reedsGovernance";
...
export type InvokeParams = {
  ...
  /** Reeds governance metadata. Existing callers may omit this during migration. */
  governance?: ReedsModelGovernanceContext;
};
...
const resolveApiUrl = () => {
  if (process.env.OPENAI_API_KEY?.trim()) return "https://api.openai.com/v1/chat/completions";
  return ENV.forgeApiUrl?.trim() ? `${ENV.forgeApiUrl.replace(/\/$/, "")}/v1/chat/completions` : "https://forge.manus.im/v1/chat/completions";
};

const resolveApiKey = () => process.env.OPENAI_API_KEY?.trim() ? process.env.OPENAI_API_KEY : ENV.forgeApiKey;

const resolveModel = () => process.env.OPENAI_API_KEY?.trim() ? "gpt-4.1-mini" : "gemini-2.5-flash";

const assertApiKey = () => {
  if (!(process.env.OPENAI_API_KEY || "") && !ENV.forgeApiKey) throw new Error("No API key configured. Set OPENAI_API_KEY or BUILT_IN_FORGE_API_KEY.");
};
...
/**
 * Single model gateway for the application.
 * Every invocation receives a Reeds event/run/correlation record before the
 * provider request and a completion/failure audit after the provider returns.
 * Provider-specific policy/routing remains the next layer above this adapter.
 */
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  assertApiKey();
  const governance = await beginReedsModelRun(params, params.governance);
  ...
  try {
    ...
    const payload: Record<string, unknown> = { model: resolveModel(), messages: messages.map(normalizeMessage) };

    if (tools?.length) payload.tools = tools;
    const normalizedToolChoice = normalizeToolChoice(toolChoice || tool_choice, tools);
    if (normalizedToolChoice) payload.tool_choice = normalizedToolChoice;

    payload.max_tokens = useOpenAI ? 4096 : 32768;
    if (!useOpenAI) payload.thinking = { budget_tokens: 128 };
    ...
    await finishReedsModelRun(governance, { model: result.model, id: result.id, usage: result.usage });
    return result;
  } catch (error) {
    await failReedsModelRun(governance, error);
    throw error;
  }
}
